...
## 📌 Project Overview
최신 AI 모델을 다루기 위한 프롬프트 기법들을 직관적으로 학습할 수 있는 **'사이버펑크/네온' 테마**의 웹사이트입니다.
...
* **글래스모피즘 네비게이션** : `backdrop-filter: blur()`와 `position: fixed`를 활용한 반투명 상단 고정 바
...
* **3D 플립 카드 (자주 하는 실수)** : `perspective`, `transform: rotateY()`, `backface-visibility`를 활용한 입체 인터랙션
...
* **무한 롤링 텍스트 (마키)** : `@keyframes`와 `transform: translateX()`를 사용해 끊임없이 흘러가는 전광판 효과
